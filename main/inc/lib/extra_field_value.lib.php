    ...
    public function __construct($type) {
        ...
        switch ($this->type) {
            case 'course':
                ...
                $this->table_handler_field = Database::get_main_table(
                    TABLE_MAIN_COURSE_FIELD
                );
                break;
            case 'user':
    ...
    public function save_field_values($params) {
        ...
        foreach ($params as $key => $value) {
            if (substr($key, 0, 6) == 'extra_') { //an extra field
                ...
                $extra_field_info = $extra_field->get_handler_field_info_by_field_variable(
                    $field_variable
                );
                if ($extra_field_info) {
                    $new_params = array(
