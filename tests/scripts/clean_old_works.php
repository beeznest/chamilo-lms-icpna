 ...
 * then delete the exit() statement at line 12.
...
$expiryDate = '2017-05-01'; //session end date must be < to be considered
$simulate = false;
if (PHP_SAPI !== 'cli') {
...
if ($resc === false) {
    die("Error querying total sessions count: ".Database::error($res)."\n");
}
...
foreach ($sessionCourses as $sid => $cid) {
    ...
    if (mysql_num_rows($resCarpetas) > 0) {
        while ($rowDemo = mysql_fetch_assoc($resCarpetas)) {
            ...
            $totalSize += $size;
            echo "Freeing $size summing to a total $totalSize bytes in $carpetaAlpElimina\n";
            if ($simulate == false) {
...
echo "[".time()."] ".($simulate ? "Would delete" : "Deleted")." tasks from $countSessions sessions between $fromDate and $expiryDate on a total of $countAllSessions sessions for a total estimated size of " . round($totalSize/(1024*1024)) . " MB."."\n";
