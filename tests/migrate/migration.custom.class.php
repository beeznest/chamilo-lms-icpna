        ...
        if (!empty($course_code)) {
            ...
            if ($data_to_update['error'] == false) {
                ...
                foreach ($data_to_update as $field => $value) {
                    if (substr($field,0,6)=='extra') { continue; } //don't deal with extra fields for now
    ...
    static function process_transaction($transaction_info, $transaction_status_list = array(), $forced = false, $deprecated = false) {
        if ($transaction_info) {
            ...
            if (empty($transaction_info)) {
                ...
                if (in_array($transaction_info['status_id'], array(0, self::TRANSACTION_STATUS_FAILED, self::TRANSACTION_STATUS_TO_BE_EXECUTED))) {
                    return array(
