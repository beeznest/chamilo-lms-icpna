...
if ($action == 'add') {
    $interbreadcrumb[] = array(
        'url' => 'exercise_fields.php',
        'name' => get_lang('ExerciseFields')
    );
...
$url = api_get_path(
        WEB_AJAX_PATH
    ) . 'model.ajax.php?a=get_extra_fields&type=exercise';
...
$(function() {
    ...
    $("#field_type").on("change", function() {
        ...
        switch(id) {
            case "1":
                $("#example").html("' . addslashes(
        Display::return_icon('userfield_text.png')
    ) . '");
                break;
            case "2":
                $("#example").html("' . addslashes(
        Display::return_icon('userfield_text_area.png')
    ) . '");
                break;
            case "3":
                $("#example").html("' . addslashes(
        Display::return_icon('add_user_field_howto.png')
    ) . '");
                break;
            case "4":
                $("#example").html("' . addslashes(
        Display::return_icon('userfield_drop_down.png')
    ) . '");
                break;
            case "5":
                $("#example").html("' . addslashes(
        Display::return_icon('userfield_multidropdown.png')
    ) . '");
                break;
            case "6":
                $("#example").html("' . addslashes(
        Display::return_icon('userfield_data.png')
    ) . '");
                break;
            case "7":
                $("#example").html("' . addslashes(
        Display::return_icon('userfield_date_time.png')
    ) . '");
                break;
            case "8":
                $("#example").html("' . addslashes(
        Display::return_icon('userfield_doubleselect.png')
    ) . '");
                break;
            case "9":
                $("#example").html("' . addslashes(
        Display::return_icon('userfield_divider.png')
    ) . '");
                break;
            case "10":
                $("#example").html("' . addslashes(
        Display::return_icon('userfield_user_tag.png')
    ) . '");
                break;
            case "11":
                $("#example").html("' . addslashes(
...
switch ($action) {
    case 'add':
        ...
        $url = api_get_self() . '?action=' . Security::remove_XSS(
                $_GET['action']
            );
    ...
    case 'edit':
        ...
        if ($form->validate()) {
            if ($check) {
                ...
                Display::display_confirmation_message(
                    sprintf(get_lang('ItemUpdated'), $values['name']),
                    false
                );
            }
            $obj->display();
