...
class Exercise
{
    ...
    /**
     * This is an implementation from Exercise::manage_answer().
     * It's used in test/scripts/english_contest_results.php
     *
     * @param        $exeId
     * @param        $questionId
     * @param        $choice
     * @param string $from
     * @param array  $exerciseResultCoordinates
     * @param bool   $saved_results
     ...
     * @todo    reduce parameters of this function
     */
    public function manageAnswerWithoutHTML(
        $exeId,
        $questionId,
        $choice,
        $from = 'exercise_show',
        ...
        // Get answer list for matching
        $sql_answer =
            'SELECT id, answer FROM '.$table_ans.' WHERE c_id = '.$course_id.' AND question_id = "'.$questionId.'" ';
            ...
            switch ($answerType) {
                ...
                case FILL_IN_BLANKS :
                    ...
                    while (1) {
                        ...
                        if ($from_database) {
                            $queryfill = "SELECT answer FROM ".$TBL_TRACK_ATTEMPT." WHERE exe_id = '".$exeId
                                ."' AND question_id= '".Database::escape_string(
                                    $questionId
                                )."'";
                ...
                case FREE_ANSWER :
                    if ($from_database) {
                        $query = "SELECT answer, marks FROM ".$TBL_TRACK_ATTEMPT." WHERE exe_id = '".$exeId
                ...
                case MATCHING:
                    if ($from_database) {
                        $sql_answer =
                            'SELECT id, answer FROM '.$table_ans.' WHERE c_id = '.$course_id.' AND question_id="'
                            .$questionId.'" AND correct=0';
                        $res_answer = Database::query($sql_answer);
                        // getting the real answer
                        $real_list = array();
                        while ($real_answer = Database::fetch_array($res_answer)) {
                        ...
                        $sql_select_answer = 'SELECT id, answer, correct, id_auto FROM '.$table_ans.'
                                              WHERE c_id = '.$course_id.' AND question_id="'.$questionId
                            .'" AND correct <> 0 ORDER BY id_auto';
                            ...
                            if (!empty($s_user_answer)) {
                                if ($s_user_answer == $i_answer_correct_answer) {
                                    ...
                                    if ($answerType == DRAGGABLE) {
                                        ...
                                        $user_answer = '<span style="color: #FF0000; text-decoration: line-through;">'
                                            .$real_list[$s_user_answer].'</span>';
                                    }
                                }
                            } else {
                                if ($answerType == DRAGGABLE) {
                ...
                case HOT_SPOT :
                    if ($from_database) {
                        $TBL_TRACK_HOTSPOT = Database::get_main_table(
                ...
                case HOT_SPOT_DELINEATION :
                    if ($from_database) {
                        // getting the user answer
                        $TBL_TRACK_HOTSPOT = Database::get_main_table(TABLE_STATISTIC_TRACK_E_HOTSPOT);
                        $query = "SELECT hotspot_correct, hotspot_coordinate from ".$TBL_TRACK_HOTSPOT
                            ." where hotspot_exe_id = '".$exeId."' and hotspot_question_id= '".$questionId
                            ."' AND hotspot_answer_id='1'"; //by default we take 1 because it's a delineation
                        $resq = Database::query($query);
                        $row = Database::fetch_array($resq, 'ASSOC');
