...
function search_users($needle, $type) {
    ...
    if (!empty($type)) {
        ...
        if ($_configuration['multiple_access_urls']) {
			...
			WHERE  " . (api_sort_by_first_name() ? 'firstname' : 'lastname') . " LIKE '{$prePerc}$needle%' AND status NOT IN(" . DRH . ", " . SESSIONADMIN . ") AND user.user_id NOT IN ($user_anonymous, $current_user_id, $user_id) $without_assigned_users AND access_url_id = " . api_get_current_access_url_id() . "";
        } else {
            $sql = "SELECT user_id, username, lastname, firstname FROM $tbl_user user
			WHERE  " . (api_sort_by_first_name() ? 'firstname' : 'lastname') . " LIKE '{$prePerc}$needle%' AND status NOT IN(" . DRH . ", " . SESSIONADMIN . ") AND user_id NOT IN ($user_anonymous, $current_user_id, $user_id) $without_assigned_users";
        }
...
?>
<form class="form-horizontal" name="formulaire" id="formulaire" method="post" action="<?php echo api_get_self(); ?>?user=<?php echo $user_id ?>" style="margin:0px;" <?php if ($ajax_search) {
        ...
        <?php if ($add_type == 'multiple') { ?>
            <tr>
                <td width="45%" align="center">
                    <!--<label for="user_to_add">  <?php //echo get_lang('FirstLetterUser');  ?> :</label>-->
                    <input type="text" id="user_to_add" name="user_to_add" onkeyup="OnKeyRequestBuffer.modified('user_to_add')"/>
                </td>
                <td>&nbsp;</td>
            </tr>
        <?php } ?>
...
<script>
    var OnKeyRequestBuffer = 
        ...
        bufferText: false,
        bufferTime: 500,
        modified : function(txtId)
        {
            setTimeout('OnKeyRequestBuffer.compareBuffer("' + txtId + '");', this.bufferTime);
        },
        compareBuffer : function(txtId)
        {
            if (xajax.$(txtId).value.length > 3 || xajax.$(txtId).value.length == 0)
            {
                OnKeyRequestBuffer.makeRequest(xajax.$(txtId).value);
            }
        },
        makeRequest : function(value)
        {
            xajax_search_users(value, 'typing');
        }
    }
