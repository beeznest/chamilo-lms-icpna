...
class Sequence {
    ...
    public static function get_row_entity_id_by_row_id($entity_id, $row_id, $c_id, $session_id) {
        ...
        $session_id = Database::escape_string($session_id);
        if (is_numeric($row_id) && is_numeric($entity_id) && is_numeric($c_id) && is_numeric($session_id)) {
            $row_id = intval($row_id);
            $entity_id = intval($entity_id);
            $c_id = (intval($c_id) != 0) ? intval($c_id) : api_get_course_int_id();
            $session_id = (intval($session_id) != 0) ? intval($session_id) : api_get_session_id();
            $row_table = Database::get_main_table(TABLE_SEQUENCE_ROW_ENTITY);
    ...
    public static function temp_hack_3_insert($entity_id_prev, $entity_id_next, $row_id_prev = 0, $row_id_next = 0, $c_id = 0, $session_id = 0, $is_part = 0) {
