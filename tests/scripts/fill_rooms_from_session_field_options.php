...
$sqlFieldId = "SELECT id, field_variable FROM extra_field WHERE extra_field_type = 3";
...
$sqlSessionFieldValues = "SELECT item_id, field_id, value FROM extra_field_values WHERE field_id in ($branchField, $roomField) AND extra_field_type = 3 ORDER BY item_id";
...
// Prepare rooms array
$rooms = array();
$sql = "SELECT option_value, display_text FROM extra_field_options WHERE field_id = $roomField";
$r = Database::query($sql);
while ($row = Database::fetch_assoc($r)) {
...
foreach ($branchRoomMatch as $room => $branch) {
    // Get branch info
    $sql = "SELECT display_text FROM extra_field_options WHERE field_id = $branchField AND option_value = '$branch'";
    $r = Database::query($sql);
    $branchId = 0;
    while ($row = Database::fetch_assoc($r)) {
