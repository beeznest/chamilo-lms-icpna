...
class SocialManager extends UserManager {
     ...
     * @author isaac flores paz
     */
    public static function get_list_path_web_by_user_id(
        $user_id,
        $id_group = null,
        $search_name = null
    ) {
     ...
     * @return array
     */
    public static function get_list_web_path_user_invitation_by_user_id(
        $user_id
    ) {
    ...
    public static function send_invitation_friend_user($userfriend_id, $subject_message = '', $content_message = '') {
        ...
        if (isset($subject_message) && isset($content_message) && isset($userfriend_id)) {
            $send_message = MessageManager::send_message(
                $userfriend_id,
                $subject_message,
                $content_message
            );
    ...
    public static function show_social_menu($show = '', $group_id = 0, $user_id = 0, $show_full_profile = false, $show_delete_account_button = false) {
        ...
        if (!in_array(
            ...
            $html .= '<li class="' . $active . '"><a href="' . api_get_path(
                    WEB_PATH
                ) . 'main/social/myfiles.php">' . Display::return_icon(
                    'briefcase.png',
                    get_lang('MyFiles'),
                    array(),
                    16
                ) . get_lang('MyFiles') . '</span></a></li>';
            $html .='</ul>
                  </div>';
        }
        ...
        if ($_GET['id'] == '') {
            ...
            foreach ($user_list as $uid) {
                ...
                if ($image_array['file'] == 'unknown.jpg' || !file_exists($image_array['dir'].$image_array['file'])) {
                    $friends_profile['file'] = api_get_path(
                            WEB_CODE_PATH
                        ) . 'img/unknown_180_100.jpg';
