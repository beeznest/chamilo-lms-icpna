...
class Migration
{
     ...
     * object
     * @param string $dbhost The original database's host
     * @param string $dbport The original database's port
     * @param string $dbuser The original database's user
     * @param string $dbpass The original database's password
     * @param string $dbname The original database's name
     ...
     * @param array $matches Structured array of matches (see migrate.php)
     */
     ...
     * @param Array $table An array containing an 'extra_fields' entry with details about the required extra fields
     * @return void
     */
     ...
     * @param array $table List of fields that must be matched ('fields_match' => array(0=>array('orig'=>'...','dest'=>'...',...)))
     ...
     * @param array $extra_fields Extra fields table definition
     * @return array|bool
     */
     ...
     * @param bool $truncate Whether to truncate the transaction table before the test or not
     * @return void
     */
    ...
    /**
     * Clean all transactions from the branch_transaction table.
     * After all, they're supposed to be executed at some point...
     * @return void
     */
     ...
     * @param array $params The transaction details (array('id' => ..., 'action' => '...', ...))
     ...
     * @return int The operation results
     */
    ...
    {
        $table = Database::get_main_table(TABLE_BRANCH);
        $sql = "SELECT DISTINCT(id) as branch_id FROM $table ORDER BY id";
        $result = Database::query($sql);
     ...
     * @return int The ID of the last transaction registered, or 0 if none found
     */
    static function get_latest_transaction_id_by_branch($branch_id)
    {
        ...
        return 0; //change when migrating to put latest migration transaction to start from
    }

    /**
     * Loads a specific set of transactions from the transactions table and executes them
     * @param array $params Transactions filter
     * @return int
     */
                    ...
                    if (!empty($transactions)) {
                        foreach ($transactions as $transaction) {
                            ...
                            if ($transaction['id'] == $latest_id_attempt) {
                                $latest_id_attempt++;
                                error_log("Transaction #$latest_id_attempt is latest on record in branch #{$branch_info['branch_id']} \n");
                            } else {
     ...
     * @param array $params Select parameters (associative array)
     * @param string $type_result Type of result set expected
    ...
    function execute_transaction($transaction_info)
    {
        ...
        if (isset($validate['error']) && $validate['error']) {
            if ($transaction_info['failed_attempts'] >= 3) {
                self::update_transaction(array(
                    'id' => $transaction_info['id'],
                    'failed_attempts' => $transaction_info['failed_attempts'] + 1,
                    'status_id' => 5
                ));
            } else {
                self::update_transaction(array(
        ...
        error_log("-----------------------------------------------------------");
        error_log("Executing transaction ".$transaction_info['id'].' (tid '.$transaction_info['transaction_id'].' from branch '.$transaction_info['branch_id'].')');
        error_log("-----------------------------------------------------------");
        ...
        if (method_exists('MigrationCustom', $function_to_call)) {
            ...
            // YYYY uncomment next line to get full dump of function response
                ...
                if (isset($result['error']) && $result['error'] == true) {
                    if ($transaction_info['failed_attempts'] >= 3) {
                        ...
                        self::update_transaction(array(
                            'id' => $transaction_info['id'],
                            'status_id' => 5,
                            'failed_attempts' => $transaction_info['failed_attempts']
                        ));
                    } else {
    ...
    static function update_transaction($params)
    {
        ...
        //local declaration (temporary) of constants declared in migration.custom.class.php
        $status = array(
            1 => 'pending',
            2 => 'executed',
            3 => 'deprecated',
        ...
        if ($params['status_id'] == MigrationCustom::TRANSACTION_STATUS_FAILED) {
            ...
            Event::addEvent('transaction_error', 'transaction_id', $params['id'], $params['time_update']);
        }
        ...
        if ($result && isset($result[0])) {
            ...
            if ($result['idt'] == $transaction_external_id) {
                ...
                if ($transaction_result['error'] == false) {
                    ...
                    $transaction_chamilo_info = self::get_transaction_by_params(array(
                        'Where' => array('id = ?' => $chamilo_transaction_id),
                        'first'
                    ));
                    ...
                    $message .= Display::return_message("Transaction NOT added to Chamilo. {$transaction_result['message']}",
                        'warning');
                }
                ...
                return array(
                    ...
                    //Display::page_subheader2("Transactions:").
                        Display::page_subheader3("Chamilo transaction info:")."<pre>".print_r($transaction_chamilo_info,
                            true)."</pre>".
                        Display::page_subheader3("Webservice transaction reponse:")."<pre>".print_r($result,
                            true)."</pre>",
                );
            }
        }
     ...
     * @param array $web_service_params Settings for the WS call
     * @param string $function_name Name of the function to call
     * @param array $params Variables to be passed as params to the function
     * @return array|bool Results as returned by the SOAP call (or false on error)
     */
    static function soap_call($web_service_params, $function_name, $params = array())
    {
        ...
        try {
            ...
            //$client = new SoapClient($url, array('cache_wsdl' => WSDL_CACHE_NONE));//, 'exceptions' => 1));
        } catch (SoapFault $fault) {
            error_log('Error instanciating new SOAP client - exception caught: '.$fault->getMessage());
            $error = 1;
            return false;
